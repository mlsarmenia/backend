...
@php
    ...
    $editableValue = old_empty_or_null($field['name'], []) ??  $field['value'] ?? $field['default'] ?? [];
    ...
    $activeInlineEditLink = !empty($field['inline_edit_link']) ? true : false;
...
@include('crud::fields.inc.wrapper_start')
        ...
        @endif
        @if($activeInlineCreate && $routeEntity && $editableValue)
                @php
                    $inlineCreateEntity = $field['inline_create']['entity'];
                @endphp
                <a href="{{ url('/admin/'.($inlineCreateEntity ?: $routeEntity).'/'.$editableValue.'/edit') }}"
                   class="btn btn-sm btn-link inline-edit-button float-right" target="_blank"><i class="la la-edit"></i> Խմբագրել</a>
        @endif
