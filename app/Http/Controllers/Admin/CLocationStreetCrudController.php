<?php

namespace App\Http\Controllers\Admin;

use App\Http\Controllers\Admin\Traits\CrudPermissionTrait;
use App\Http\Requests\CLocationStreetRequest;
use App\Models\CLocationStreet;
use Backpack\CRUD\app\Http\Controllers\CrudController;
use Backpack\CRUD\app\Library\CrudPanel\CrudPanelFacade as CRUD;
use Backpack\CRUD\app\Library\Widget;
use Illuminate\Foundation\Auth\Access\AuthorizesRequests;

/**
 * Class CLocationStreetCrudController
 * @package App\Http\Controllers\Admin
 * @property-read \Backpack\CRUD\app\Library\CrudPanel\CrudPanel $crud
 */
class CLocationStreetCrudController extends CrudController
{
    use \Backpack\CRUD\app\Http\Controllers\Operations\ListOperation;
    use \Backpack\CRUD\app\Http\Controllers\Operations\CreateOperation;
    use \Backpack\CRUD\app\Http\Controllers\Operations\UpdateOperation;
    use \Backpack\CRUD\app\Http\Controllers\Operations\DeleteOperation;
    use \Backpack\CRUD\app\Http\Controllers\Operations\ShowOperation;
    use AuthorizesRequests;
    use CrudPermissionTrait;

    /**
     * Configure the CrudPanel object. Apply settings to all operations.
     *
     * @return void
     */
    public function setup()
    {
        CRUD::setModel(CLocationStreet::class);
        CRUD::setRoute(config('backpack.base.route_prefix') . '/c-location-street');
        CRUD::setEntityNameStrings('փողոց', 'փողոցներ');
        $this->setAccessUsingPermissions(CLocationStreet::class);
    }

    /**
     * Define what happens when the List operation is loaded.
     *
     * @see  https://backpackforlaravel.com/docs/crud-operation-list-entries
     * @return void
     */
    protected function setupListOperation()
    {
        CRUD::column('id');
        CRUD::column('name_arm');
        CRUD::column('name_eng');
        CRUD::column('name_ru');
        CRUD::column('created_at');
        CRUD::column('updated_at');
    }

    /**
     * Define what happens when the Create operation is loaded.
     *
     * @see https://backpackforlaravel.com/docs/crud-operation-create
     * @return void
     */
    protected function setupCreateOperation()
    {
        CRUD::setValidation(CLocationStreetRequest::class);
        Widget::add()->type('script')->content('assets/js/admin/forms/street.js');

        CRUD::addField([
            'name' => 'name_arm',
            'label' => 'Անուն',
        ]);
        CRUD::addField([
            'name' => 'name_eng',
            'label' => 'Անուն (անգլերեն)',
        ]);
        CRUD::addField([
            'name' => 'name_ru',
            'label' => 'Անուն (ռուսերեն)',
        ]);
        CRUD::addField([
            'name' => 'parent_is_community',
            'label' => 'Տարածք',
            'type' => 'radio',
            'options' => [
                0 => 'Քաղաք',
                1 => 'Համայնք',
            ],
            'default' => 0,
            'inline' => true,
            'value' => $this->crud->getCurrentEntry() ? (int) $this->crud->getCurrentEntry()->parent_is_community : null,
        ]);
        CRUD::addField([
            'name' => 'location_city',
            'type' => 'relationship',
            'attribute' => 'name_arm',
            'minimum_input_length' => 0,
            'include_all_form_fields' => true,
            'label' => 'Քաղաք',
            'placeholder' => '-Ընտրել մեկը-',
            'wrapper' => [
                'class' => 'form-group col-md-3',
            ],
        ]);
        CRUD::addField([
            'name' => 'location_community',
            'type' => 'relationship',
            'attribute' => 'name_arm',
            'minimum_input_length' => 0,
            'label' => 'Համայնք',
            'placeholder' => '-Ընտրել մեկը-',
            'wrapper' => [
                'class' => 'form-group col-md-3',
            ],
        ]);
    }

    /**
     * Define what happens when the Update operation is loaded.
     *
     * @see https://backpackforlaravel.com/docs/crud-operation-update
     * @return void
     */
    protected function setupUpdateOperation()
    {
        $this->setupCreateOperation();
    }

    protected function setupShowOperation()
    {
        $this->setupCreateOperation();
    }
}
