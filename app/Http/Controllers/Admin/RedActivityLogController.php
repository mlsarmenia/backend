 ...
 */
class RedActivityLogController extends ActivityLogCrudController
{
    const CAUSER = 'causer';
...
//        CRUD::setEntityNameStrings(__('backpack.activity-log::activity_log.activity_log'), __('backpack.activity-log::activity_log.activity_logs'));
        CRUD::setEntityNameStrings('Փոփոխություն', 'Փոփոխություններ');
        $this->setAccessUsingPermissions(ActivityLog::class);
    }
    ...
    protected function setupListOperation()
    {
        CRUD::addColumn([
            ...
            'searchLogic' => function ($query, $column, $searchTerm) {
                $query->orWhere('properties', 'like', '%' . $searchTerm . '%')->orWhere('subject_id', 'like', '%' . $searchTerm . '%');
            },
        ])->beforeColumn('causer');

        CRUD::addColumn([
            'name' => 'causer_type',
            'label' => 'Օգտատիրոջ տեսակ',
            'type' => 'text',
            'className' => 'col-md-7 mb-4',
        ...
        CRUD::addColumn([
            ...
            'type' => 'text',
            'className' => 'col-md-7 mb-4',
            'value' => fn($entry) => $entry->causer && method_exists($entry->causer, 'identifiableAttribute') ? $entry->causer->{$entry->causer->identifiableAttribute()}.' ('.$entry->causer->email.')' : '',
            'wrapper' => [
        ...
        CRUD::addColumn([
            ...
            'type' => 'text',
            'className' => 'col-md-7 mb-4',
//            'value' => fn($entry) => $entry->subject && method_exists($entry->subject, 'identifiableAttribute') ? $entry->subject->{$entry->subject->identifiableAttribute()}.' - '.$entry->subject->id : '',
            'value' => fn($entry) => $entry->subject && method_exists($entry->subject, 'identifiableAttribute') ? $entry->subject->id : '',
            'wrapper' => [
